---
title: "About"
layout: "about"
description: "Kamil Tagowski, PhD, postdoc and AI engineer in graph representation learning, NLP, and computer vision at Wrocław University of Science and Technology."
expertise:
  - title: "Natural Language Processing"
    icon: "sparkle"
    summary: "Transformer models, retrieval, extraction, and evaluation for real-world and low-resource text, including Polish."
  - title: "Graph Representation Learning"
    icon: "graph"
    summary: "Embeddings and graph models for entities, documents, and the relationships between them."
  - title: "Vision & Generative AI"
    icon: "image"
    summary: "Vision-language models and generative image models: reading and synthesizing visual content, from game-map tiles to document layouts."
  - title: "Legal & Document AI"
    icon: "briefcase"
    summary: "Understanding, search, and reasoning over large corpora of legal and administrative documents."
  - title: "Benchmarks & Evaluation"
    icon: "check"
    summary: "Designing datasets, metrics, and reproducible harnesses, from human annotation to LLM-as-judge."
  - title: "Datasets & Pipelines"
    icon: "dataset"
    summary: "Versioned data infrastructure with DVC, active learning, and human-in-the-loop curation."
  - title: "Research Engineering"
    icon: "code"
    summary: "Turning research prototypes into maintainable systems with FastAPI, PyTorch, and modern tooling."
timeline:
  - year: "2022"
    title: "LEPISZCZE benchmark"
    place: "NeurIPS 2022 · Datasets & Benchmarks"
    summary: "Co-authored a reproducible benchmark for Polish NLP across 14 tasks."
  - year: "2023–25"
    title: "Legal & tax AI at scale"
    place: "AI-Tax / Legal AI"
    summary: "Built production systems indexing millions of legal documents for researchers and professionals."
  - year: "2024–26"
    title: "Unite! researcher discovery"
    place: "UNITE! European university alliance"
    summary: "Semantic search over hundreds of thousands of researcher profiles across nine universities."
  - year: "Now"
    title: "Postdoc & AI Engineer"
    place: "Wrocław University of Science and Technology"
    summary: "Graph ML and NLP for document understanding, with one foot in research and one in production."
---

I work at the seam between research and engineering. My academic background is in **graph representation learning** and **natural language processing**, and my day-to-day is building systems that take those ideas out of papers and into tools people rely on.

That has meant co-authoring **LEPISZCZE**, a NeurIPS 2022 benchmark that standardized how Polish language models are evaluated, and it has meant shipping production **legal and tax AI** platforms that index millions of documents and serve working researchers. I care about the unglamorous parts that decide whether a model is actually useful: clean datasets, honest evaluation, and software that does not fall over.

More recently that work has grown to include **vision and generative models**: vision-language systems that read imagery and generative models that create it, like an AI pipeline for generating and editing game maps.

If you are working on something at the intersection of language, graphs, vision, and real-world documents, I would like to hear about it.
